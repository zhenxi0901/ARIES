...
4. [x] Give the harness the gateway through `harness.mcp_servers` (with the
   SSE transport and a per-call timeout), so it reaches the gateway at the
   sandbox's fixed network alias.
