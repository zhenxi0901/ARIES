...
- **The harness** reaches the gateway at `http://task-sandbox:<port>/sse`,
  the sandbox's fixed network alias, through a `harness.mcp_servers`
  entry. Only the Hermes harness renders an MCP client configuration today,
...
"harness": {
  "type": "hermes",
  "mcp_servers": [{"name": "toolathlon", "url": "http://task-sandbox:10086/sse", "transport": "sse", "timeout_seconds": 300}]
}
```
...
defaults. The gateway port in `harness.mcp_servers[].url` must match
