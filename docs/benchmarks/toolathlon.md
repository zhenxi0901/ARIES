...
- **The harness** reaches the gateway at `http://task-sandbox:<port>/sse`,
  ...
  task's business, and they all sit behind the one gateway). The adapter
  is wired for the Hermes harness, so `benchmark.type: "toolathlon"`
  requires `harness.type: "hermes"`; OpenClaw follows. The
