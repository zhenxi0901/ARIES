...
| Benchmark | **Toolathlon** — 108 tool-use tasks over MCP servers; the adapter serves the subset whose servers are local, public, or backed by Toolathlon's self-hosted applications (Canvas, poste.io, WooCommerce) | Verifies the pinned checkout, loads tasks, installs the pinned project tree and runs Toolathlon's own preprocess and MCP gateway inside the sandbox, stashes the grader and ground truth on the host before bridge access, and grades with Toolathlon's own evaluator after isolation | `benchmark.type: "toolathlon"`; checkout and revision in `configs/versions.json`; requires `benchmark.environment.image` (Toolathlon's task image) and `harness.type: "hermes"` (the gateway is added to the harness's MCP client by the adapter); optional `benchmark.toolathlon` block; `profiles/hermes-toolathlon-canvas-list-test-deepseek.json`; [benchmark guide](benchmarks/toolathlon.md) |
